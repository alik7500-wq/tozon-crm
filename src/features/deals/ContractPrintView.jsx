...
export const ContractPrintView = ({ deal, onClose, initialLang = 'TJ' }) => {
  const [activeTab, setActiveTab] = useState('CONTRACT'); // 'SCHEDULE' or 'CONTRACT'
  ...
  // USD / TJS conversions for clause 3.1
  const usdPrice = currency === 'USD' ? pricePerM2Number : Math.round(pricePerM2Number / 10.9);
  const tjsPrice = currency === 'TJS' ? pricePerM2Number : Math.round(pricePerM2Number * 10.9);
  ...
  return createPortal(
    <div className="print-portal-root fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs p-2 sm:p-6 flex justify-center animate-in fade-in print:static print:p-0 print:m-0 print:bg-white print:overflow-visible print:block print:h-auto">
      <div className="print-document-root relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col my-auto border border-slate-200 print:border-none print:shadow-none print:rounded-none print:max-w-none print:w-full print:overflow-visible print:m-0 print:p-0 print:static print:block print:h-auto">
        ...
        {/* ================= TAB 1: МАТНИ ШАРТНОМА (EXACT 3-PAGE LEGAL CONTRACT FROM IMAGE) ================= */}
        {activeTab === 'CONTRACT' && (
          <div id="print-section" className="p-8 sm:p-14 text-slate-950 text-xs sm:text-[13px] leading-relaxed font-serif select-text bg-white space-y-5">
            ...
            <div className="text-center pb-2">
              <h1 className="text-base sm:text-lg font-black tracking-tight uppercase text-slate-950 leading-snug">
                {isTJ ? (
                  <>
                    ШАРТНОМАИ ИШТИРОКИ ҲИССАГӮЙ ДАР СОХТМОНИ БИНОИ ИСТИҚОМАТӢ № <u className="font-mono">{deal.contract_number || '____'}</u>
                  </>
                ) : (
                  <>
            ...
            <div className="flex justify-between items-baseline text-xs sm:text-[13px] font-bold pb-2">
              <span>
                «<u className="font-mono">{dealDay}</u>» <u>{dealMonthName}</u> {isTJ ? `соли ${dealYear}` : `${dealYear} года`}
              </span>
            ...
            <div className="text-justify text-xs sm:text-[13px] leading-relaxed">
              {isTJ ? (
                <p>
                  <strong>{cleanCompanyName(deal.developer_name)}</strong>, дар шахси директор <strong>{deal.developer_director || 'Иброгимов Илхомчон Мукимович'}</strong>, ки дар асоси Оиннома амал менамояд минбаъд «<strong>Ширкати сохтмонӣ</strong>» номида мешавад аз як тараф ва аз дигар тараф шаҳрванди Ҷумҳурии Тоҷикистон, <strong><u>{deal.lead_name || deal.buyer_name || '____________________'}</u></strong> дар асоси «<strong>Ҳиссагузор</strong>» дорандаи шиносномаи <strong><u>{deal.passport_series ? `${deal.passport_series} ` : ''}{deal.passport_number || '________'}</u></strong>, ки аз тарафи <strong><u>{deal.passport_issued_by || '____________________'}</u></strong>, санаи <strong><u>{formatDate(deal.passport_issue_date) || '_________'}</u></strong> сол дода шудааст, сокини <strong><u>{deal.registration_address || '____________________'}</u></strong>, РМА <strong><u>{deal.inn || deal.passport_number || '__________'}</u></strong> дар якҷоягӣ «<strong>Тарафҳо</strong>» номида мешаванд, дар натиҷаи машварати гузаронида шуда, бо назардошти талаботи боби 60 Кодекси граждании Ҷумҳурии Тоҷикистон бо мақсади фаъолияти якҷоя, шартномаи зеринро бастанд.
                </p>
              ) : (
                <p>
                  <strong>{cleanCompanyName(deal.developer_name)}</strong>, в лице директора <strong>{deal.developer_director || 'Иброгимова Илхомджона Мукимовича'}</strong>, действующего на основании Устава, именуемое в дальнейшем «<strong>Строительная компания</strong>», с одной стороны, и гражданин Республики Таджикистан <strong><u>{deal.lead_name || deal.buyer_name || '____________________'}</u></strong>, именуемый(ая) в дальнейшем «<strong>Дольщик</strong>», паспорт: <strong><u>{deal.passport_series ? `серия ${deal.passport_series} ` : ''}№ {deal.passport_number || '________'}</u></strong>, выдан <strong><u>{deal.passport_issued_by || '____________________'}</u></strong>, дата выдачи <strong><u>{formatDate(deal.passport_issue_date) || '_________'}</u></strong> г., проживающий(ая) по адресу: <strong><u>{deal.registration_address || '____________________'}</u></strong>, ИНН: <strong><u>{deal.inn || deal.passport_number || '__________'}</u></strong>, совместно именуемые «<strong>Стороны</strong>», заключили настоящий Договор о нижеследующем:
                </p>
              )}
            </div>
            ...
            <div className="space-y-1.5 text-justify text-xs sm:text-[13px] leading-relaxed">
              ...
              <p className="pl-4">
                {isTJ
                  ? '1.1.1. Барои бастан ва ба иҷро расонидани шартномаи мазкур ҳамаи ҳуҷҷатҳои заруриро дастрасии, иҷозатномаи ва ё дигар ҳуҷҷатҳо ва ё шартномаҳо, ки аз тарафи мақомотҳо ва шахсони дахлдор ба онҳо ваколат дода шудааст, «Ширкати сохтмонӣ» гирифта баста аст ва онҳо ба қувваи қонуни даромада амал мекунанд.'
              ...
              <p className="pl-4">
                {isTJ
                  ? '1.1.3. Хонаи бисёрошёна ҳатман эътирозкунии сохтмони ҳиссавӣ баъди қабули санади комиссияи давлатӣ ба «Ҳиссагузор» супорида мешавад.'
            ...
            <div className="space-y-1.5 text-justify text-xs sm:text-[13px] leading-relaxed">
              ...
              <p>
                {isTJ ? (
                  <>
                    2.1. «Тарафҳо» оиди тайёр намудан ва дар амалӣ ба анҷом расонидани шартномаи сохтмони шарикии хонаи бисёрошёна, ки дар суроғаи <strong><u>{deal.project_address || deal.project_name || 'шаҳри Хуҷанд, кӯчаи Бахтиёр Ёқубов'}</u></strong> ҷойгир мебошад, аҳду паймон кардаанд. Дар асоси шартнома «Ширкати сохтмонӣ» сармоягузориро баромад менамояд ва «Ҳиссагузор» ҳамчун маблағгузор бошад менамояд ва дар оянда «Ҳиссагузор» соҳибмулки соҳибҳуқуқи хонаи <strong><u>{deal.unit_rooms || '—'}</u></strong> ҳуҷрагӣ, <strong><u>{deal.building_name ? `Блоки ${deal.building_name}` : deal.section_name ? `Сексияи ${deal.section_name}` : 'Блоки А'}</u></strong>, ошёнаи <strong><u>{deal.floor_number || '—'}</u></strong> дар рақами хонаи <strong><u>{deal.unit_number || '—'}</u></strong> бо масоҳати тахминии <strong><u>{areaM2}</u></strong> м² ҷойгир мебошад мушарраф мегардад.
                  </>
                ) : (
                  <>
            ...
            <div className="space-y-1.5 text-justify text-xs sm:text-[13px] leading-relaxed">
              ...
              <p>
                {isTJ ? (
                  <>
                    3.1. Бо маслиҳати «Тарафҳо» нархи 1 м.кв. хонаи истиқоматиро ба <strong><u>{pricePerM2Formatted} ({pricePerM2Words}) {currencyShort}</u></strong> ба рӯи қурби муайян шуда баробар ба <strong><u>{usdPrice} доллари ИМА</u></strong> ба рӯи қурби муайян намудани бонки миллии Ҷумҳурии Тоҷикистон нисбати 1 доллари ИМА, ба ҳолати рӯзи имзогузории шартномаи мазкур, ташкил медиҳад нархгузорӣ намудаанд.
                  </>
                ) : (
                  <>
                    3.1. По соглашению Сторон цена 1 кв.м квартиры определена в размере <strong><u>{pricePerM2Formatted} ({pricePerM2Words}) {currencyShort}</u></strong>, что эквивалентно <strong><u>{usdPrice} долларам США</u></strong> по курсу Национального банка Таджикистана на день заключения настоящего Договора. Общая стоимость составляет <strong><u>{finalPrice} {currencyShort}</u></strong>.
                  </>
                )}
              </p>
              <p>
            ...
            <div className="space-y-1.5 text-justify text-xs sm:text-[13px] leading-relaxed">
              ...
              <p className="pl-4">
                {isTJ
                  ? '4.1.1. «Ширкати сохтмонӣ» ӯҳдадор аст объекти сохтмони ҳиссагии бинои истиқоматиро бо назардошти он сифате, ки дар шартҳои шартнома ва талаботи лоиҳавию ҳуҷҷатномаи ҳуҷҷатҳо пешбинӣ шудааст, ба ҳиссагузор пешниҳод намояд.'
                  : '4.1.1. Обеспечить строительство объекта в соответствии со строительными нормами и утвержденной проектной документацией.'}
              </p>
              <p className="pl-4">
                {isTJ ? (
                  <>
                    4.1.2. Аз ҳисоби маблағҳои худ, маблағҳои иштирокчиён (саҳмгузорон) ва дигар воситаҳои ҷалбшуда, сохтмони воқеъ дар суроғаи <strong><u>{deal.project_address || deal.project_name || 'шаҳри Хуҷанд, кӯчаи Бахтиёр Ёқубов'}</u></strong>, айнан аз рӯи ҳуҷҷатҳои сохтани таъмин намуда ва аз рӯи ҷадвали муайян карда шуда ба анҷом расонад.
                  </>
                ) : (
                  <>
                    4.1.2. За счет собственных и привлеченных средств завершить строительство жилого дома по адресу: <strong><u>{deal.project_address || deal.project_name || 'г. Худжанд'}</u></strong>.
                  </>
                )}
              </p>
              <p className="pl-4">
                {isTJ
                  ? '4.1.3. Супоридани хонаи истиқоматии баландошёнаро баъди ба тамом расонидани ҳуҷҷатҳои дахлдор таъмин намояд.'
                  : '4.1.3. Передать квартиру Дольщику после оформления всей необходимой разрешительной документации.'}
              </p>
              <p className="pl-4">
                {isTJ
                  ? '4.1.4. Ҳуқуқи моликияти иштирокчиёни сохтмони ҳиссавӣ ба объекти сохтмони ҳиссавӣ ва ҳуқуқи моликият хонаи истиқоматии баландошёна дар мақомоти бақайдгирӣ ба қайд мемонад.'
                  : '4.1.4. Содействовать государственной регистрации права собственности Дольщика на приобретенную недвижимость.'}
              </p>
              <p className="pl-4">
                {isTJ ? '4.1.5. «Ширкати сохтмонӣ» хароҷоти шиносномаи техникиро ба ӯҳда мегирад.' : '4.1.5. Расходы по первичному техническому паспорту несет Строительная компания.'}
              </p>

              <p className="pt-1">{isTJ ? '4.2. «Ҳиссагузор» ба худ ӯҳдадориҳои зеринро қабул менамояд:' : '4.2. «Дольщик» обязуется:'}</p>
              <p className="pl-4">
                {isTJ
                  ? '4.2.1. Пардохт кардани арзиши хонаи баландошёна ба «Ширкати сохтмонӣ» бо тариқи пули нақд якбора, ё ин ки бо созиши тарафҳо-пардохт кардани ҳиссаи худро қисм ба қисм дар асоси ҷадвали пардохт (Замима №1), ки ҷудонашавандаи шартномаи мазкур мебошад ба хазинаи «Ширкати сохтмонӣ» дар давоми сохтмони хонаи баландошёна баробари арзиши ҳиссаи худ пардохт менамояд.'
                  : '4.2.1. Своевременно вносить оплату стоимости квартиры единовременно либо в рассрочку согласно согласованному Графику платежей (Приложение №1).'}
              </p>
              <p className="pl-4">
              ...
              <ul className="pl-8 list-disc space-y-0.5">
                ...
                <li>{isTJ ? 'ба расмият дарории ҳуҷҷатҳоро аз ҳисоби худ маблағгузорӣ менамояд.' : 'расходы по нотариальному удостоверению.'}</li>
              </ul>
              <p className="pl-4">
              ...
              <p>
                {isTJ
                  ? '4.3. «Тарафҳо» ҳуқуқ доранд ба шартҳои Шартнома тағйирот ва иловаҳо дароранд, ки бо тартиби суд бекор намоянд.'
                  : '4.3. Все изменения и дополнения к Договору оформляются в письменной форме.'}
              </p>
              <p>
                {isTJ
                  ? '4.4. «Ҳиссагузор» дар вақт бе розигии «Ширкати сохтмонӣ» метавонад ҳуқуқи худро гузашт намояд ва ӯҳдадориҳое, ки дар Шартнома дарҷ гардидааст, ба шахси дигар диҳад. «Ширкати сохтмонӣ» вазифадор аст, ки дар мӯҳлати 25 рӯз баъд аз гирифтани огоҳнома аз «Ҳиссагузор», розигӣ ё норозигии худро ба «Ҳиссагузор» хабардор намояд. Тартиб додани шартномаи иловагӣ музднок буда «Ҳиссагузор» 1% аз маблағи умумии хона пардохт намояд.'
                  : '4.4. Переуступка прав требования третьим лицам осуществляется с письменного согласия Строительной компании с уплатой 1% за оформление переуступки.'}
              </p>
            </div>
            ...
            <div className="space-y-1.5 text-justify text-xs sm:text-[13px] leading-relaxed">
              ...
              <p>
                {isTJ
                  ? '5.1. Дар ҳолате, ки агар яке аз тарафҳо аз иҷрои шартҳои мазкур саркашӣ намояд, тарафи дуюм ҳуқуқ дорад, ки бо тартиби муайян ба суд ба талаботи маҷбур намудани тарафи дигар оиди иҷрои ӯҳдадориҳо муроҷиат намояд.'
                  : '5.1. В случае уклонения одной из Сторон от исполнения обязательств, спор подлежит разрешению в судебном порядке.'}
              </p>
              <p>
                {isTJ
                  ? '5.2. Дар ҳолати бе гуноҳи яке аз тарафҳо иҷрона шудани ӯҳдадориҳои шартномавӣ яке аз Тарафҳо аз Тарафи айбдор ҷарима дар ҳаҷми 0,5% аз маблағи хонаи истиқоматии баландошёна барои ҳар як рӯзи ба таъхир гузошта шуда, рӯёнида мешавад. Айби «Ҳиссагузор» ин даст кашидан ё худ дар канор кашидан аз иҷро кардани ӯҳдадориҳо, инчунин рад намудани талаботҳои мувофиқи қарор шудагон аз оиди ҳисоби пардохти шартномаи мазкур мебошад. Айби «Ширкати сохтмонӣ» ин даст кашидан ё худ дар канор кашидан аз иҷро кардани ӯҳдадориҳои шартнома.'
                  : '5.2. За нарушение сроков исполнения обязательств виновная Сторона уплачивает неустойку в размере 0,5% за каждый день просрочки.'}
              </p>
            </div>
            ...
            <div className="space-y-1.5 text-justify text-xs sm:text-[13px] leading-relaxed">
              ...
              <p>
                {isTJ
                  ? '6.2. «Ҳиссагузор» ӯҳдадор аст, ки дар бораи ба вуҷуд омадани зуҳуроти моли ва дигар ҳолатҳои иҷро карда натавонистани ӯҳдадории маблағгузории ҳиссаи худ барои сохтмон ва бастани шартномаи мазкур, фавран роҳбарияти «Ширкати сохтмонӣ»-ро огоҳ намояд.'
                  : '6.2. Дольщик обязан незамедлительно уведомлять компанию о возникновении обстоятельств, препятствующих оплате.'}
              </p>
              <p>
                {isTJ
                  ? '6.3. Бо розигии «Тарафҳо» маблағи пулие, ки «Ҳиссагузор» пардохт намудааст барои ҳисоб намудани ӯҳдадориҳои иштирокчии нав ба ҳисоб гирифта мешавад.'
                  : '6.3. Внесенные средства могут быть зачтены новому участнику при переуступке.'}
              </p>
              <p>
                {isTJ
                  ? '6.4. Дар ҳолати «Ширкати сохтмонӣ» розигии худро оид ба гузашт намудани ҳуқуқ ва гузаронидани ӯҳдадориҳо ба шахсос, ки «Ҳиссагузор» пешниҳод намудааст, надиҳад, «Ширкати сохтмонӣ» вазифадор мешавад, ки ба «Ҳиссагузор» маблағҳои аз ӯ гирифташударо дар мӯҳлати 3 моҳ аз рӯи гирифтани огоҳиномаи хаттии «Ҳиссагузор» оиди гузашт намудани ҳуқуқ ва пешниҳод намудани ҳуҷҷатҳои тасдиқкунандаи қобили амал будани иштирокчии нав бо тарҳи ҷаримаи пешбинишуда дар ҳаҷми 20% аз маблағи пардохтшуда баргардонад.'
                  : '6.4. При невозможности согласования переуступки возврат средств производится в течение 3 месяцев с удержанием 20% штрафа.'}
              </p>
              <p>
                {isTJ
                  ? 'Агар «Ҳиссагузор» ҳуҷҷатҳои тасдиқкунандаи қобили амал будани иштирокчии навро пешниҳод карда натавонад, «Ширкати сохтмонӣ» ҳуқуқ дорад маблағи «Ҳиссагузор»-ро дар мӯҳлати оқилона баъд аз ёфтани аз тарафи «Ширкати сохтмонӣ» иштирокчии нав, бо тарҳи ҷаримаи пешбинишуда баргардонад.'
                  : 'При непредоставлении документов возврат осуществляется после привлечения нового инвестора за вычетом неустойки.'}
              </p>
            </div>
            ...
            <div className="space-y-1.5 text-justify text-xs sm:text-[13px] leading-relaxed">
              ...
              <p>
                {isTJ
                  ? '7.1. Тарафҳо барои пурра ё қисман ба иҷро нарасонидани ӯҳдадориҳо аз рӯи шартномаи мазкур аз ҷавобгарӣ озод карда мешаванд, агар расонидани иҷро ба қувваи рафънопазир алоқаманд буда, дар натиҷаи ҳолатҳои фавқулода ва ғайричашмдошт ба миён омада бошад.'
              ...
              <p>
                {isTJ ? '7.2. Тарафҳо зери мафҳуми ҳолатҳои рафънопазир чунин ҳолатҳоро мувофиқ мешуморанд:' : '7.2. К форс-мажорным обстоятельствам относятся:'}
              </p>
              <ul className="pl-6 list-disc space-y-0.5">
                <li>
                  {isTJ
                    ? 'Офатҳои табиӣ, қабули қарорҳо аз тарафи мақомоти иҷроияи маҳаллии ҳокимияти давлатӣ, ки иҷрои шартномаи мазкурро ғайриимкон месозад.'
                    : 'Стихийные бедствия, акты государственных органов, препятствующие исполнению Договора.'}
                </li>
              </ul>
            </div>
            ...
            <div className="space-y-1.5 text-justify text-xs sm:text-[13px] leading-relaxed">
              <h4 className="font-bold uppercase tracking-wider text-slate-950">
                {isTJ ? '8. ҚИСМИ ХОТИМАВӢ' : '8. ЗАКЛЮЧИТЕЛЬНЫЕ ПОЛОЖЕНИЯ'}
              </h4>
              <p>
                {isTJ
              ...
              <p>
                {isTJ
                  ? '8.3. Шартномаи мазкур ду нусха тартиб дода шудааст. Ба ҳар як нусха қувваи ҳуқуқии якхеларо доро буда, дар тарафҳо нусхаҳои онҳо дошта мешавад.'
            ...
            {/* SECTION 8 (PART 2): TABLE OF DETAILS (REQUISITES FROM IMAGE) */}
                  ...
                  <tr>
                    <td className="p-3 border-r border-slate-950 align-top space-y-2 text-slate-950">
                      ...
                      <p className="text-[11px] leading-relaxed">
                        <strong>{isTJ ? 'Суроға:' : 'Адрес:'}</strong> {deal.developer_address || 'В.Суғд, н.Б.Ғафуров, ҷамоати Ҳ.Усмонов, кӯчаи Ф. Ахмедов №150/15'}
                      </p>
                      <p className="text-[11px]"><strong>{isTJ ? 'РМА:' : 'ИНН:'}</strong> 630019325</p>
                      <p className="text-[11px]"><strong>РХМ:</strong> 6310011648</p>
                    ...
                    <td className="p-3 align-top space-y-2 text-slate-950">
                      <p className="font-extrabold text-sm">{deal.lead_name || deal.buyer_name || '____________________'}</p>
                      <p className="text-[11px] leading-relaxed">
                        <strong>{isTJ ? 'Суроға:' : 'Адрес:'}</strong> {deal.registration_address || '____________________________________'}
                      </p>
                      <p className="text-[11px] leading-relaxed">
                        <strong>{isTJ ? 'Шиноснома:' : 'Паспорт:'}</strong> {deal.passport_series ? `${deal.passport_series} ` : ''}{deal.passport_number || '________'} {isTJ ? 'аз' : 'от'} {formatDate(deal.passport_issue_date) || '_________'} {deal.passport_issued_by || '____________________'}
                      </p>
                      <p className="text-[11px]"><strong>{isTJ ? 'РМА:' : 'ИНН:'}</strong> {deal.inn || deal.passport_number || '____________________'}</p>
          ...
          <div id="print-section" className="p-6 sm:p-10 text-slate-900 text-xs sm:text-sm font-sans select-text bg-white">
            ...
            <div className="space-y-1.5 font-medium text-xs sm:text-sm pb-4">
              ...
              <div className="flex justify-between items-baseline border-b border-dotted border-slate-300 pb-0.5">
                ...
                <span className="font-bold underline text-slate-900">{pricePerM2Formatted} {currencyShort}</span>
              </div>
              ...
              <div className="flex justify-between items-baseline border-b border-dotted border-slate-300 pb-0.5">
                ...
                <span className="font-black underline text-slate-900 text-sm sm:text-base">{finalPrice} {currencyShort}</span>
              </div>
              ...
              <div className="flex justify-between items-baseline border-b border-dotted border-slate-300 pb-0.5">
                ...
                <span className="font-black underline text-slate-900 text-sm sm:text-base">{downPayment} {currencyShort}</span>
              </div>
            </div>
